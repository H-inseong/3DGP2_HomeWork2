...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	CreateCbvSrvDescriptorHeaps(pd3dDevice, 1, 17 + 50 + 1 + 1 + 3 + 7 + 1); //Billboard CBV (1), SuperCobra(17), Gunship(2), Player(1), Skybox(1), Terrain(3), Billboard(7)
	...
	m_ppShaders[1] = pBillboardShader;

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	if(pBillboardShader)
	{
		for(int i = 0; i < pBillboardShader->GetTextureCount(); i++)
		{
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[9].BaseShaderRegister = 17; // ������ �ؽ��� 7�� 17 ~ 23
	...
	pd3dRootParameters[12].Descriptor.ShaderRegister = 5;
	pd3dRootParameters[12].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[12].ShaderVisibility = D3D12_SHADER_VISIBILITY_GEOMETRY;
	...
	case WM_LBUTTONUP:
		for (int i = 0; i < m_vButtons.size(); ++i)
		{
			if (m_vButtons[i].IsClicked(x, y))
			{
				if (i == 0)
				{
					::PostMessage(hWnd, WM_SCENE_ACTION, SCENE_ACTION_START, 0);
				}
				if (i == 1)
				{
					::PostQuitMessage(0);

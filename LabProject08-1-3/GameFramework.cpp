	...
	switch (nMessageID)
	{
		case WM_KEYUP:
			switch (wParam)
			{
				...
				case VK_F2:
				case VK_F3:
					if (m_eGameState == EGameState::InGame) m_pCamera = m_pPlayer->ChangeCamera((DWORD)(wParam - VK_F1 + 1), m_GameTimer.GetTimeElapsed());
					break;
				case VK_F9:
...
void CGameFramework::BuildFont()
{
	...
	pFontTexture->LoadTextureFromDDSFile(m_pd3dDevice, m_pd3dCommandList, L"Font/D2coding_0.dds", RESOURCE_TEXTURE2D, 0);
...
void CGameFramework::BuildObjects()
{
	...
	m_vScenes[0]->m_pSpriteFont = m_pSpriteFont;
	m_vScenes[1]->m_pSpriteFont = m_pSpriteFont;
	...
	if(m_eGameState == EGameState::InGame) {
		...
		if (!bProcessedByScene)
		{
			...
			if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
			{
				...
				if (dwDirection) m_pPlayer->Move(dwDirection, 1.0f, false);
			}
		}
